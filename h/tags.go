...
// RenderPretty writes the HTML representation of the given Builder to w
// with indentation for readability. Uses two spaces for each indentation level.
// Returns nil if b is nil.
func RenderPretty(w io.Writer, b Builder) error {
	if b == nil {
		return nil
	}
	writer := NewWriter(w)
